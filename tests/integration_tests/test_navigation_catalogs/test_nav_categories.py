...
NAV_EXPECTED = FIXTURES_DIR / "test_nav_categories.json"

GROUP_1 = {
    "path": FIXTURES_DIR / "test_items_metadata_nav_categories_g1.json",
...
GROUP_2 = {
    "path": FIXTURES_DIR / "test_items_metadata_nav_categories_g2.json",
    "title": "Romance",
    "keyword": "romance",
    "total": 12219
}
...
class TestNavigationLanguage(BaseNavigationTest):
    ...
    def catalog(self, catalog_factory):
        req = CatalogRequest(CatalogType.NAVIGATION.value, nav_key="page_categories")
        return catalog_factory.build_catalog(req).model_dump(mode="json")

    def test_metadata_title(self, catalog):
        assert catalog["metadata"]["title"] == "Browse by Category"
    ...
    def test_links(self, catalog):
        ...
        assert next(l for l in catalog["links"] if l["rel"] == "self")["href"] == "/catalog?type=navigation&nav_key=page_categories"

    def test_navigation_items(self, catalog):
        assert len(catalog["navigation"]) == 14
