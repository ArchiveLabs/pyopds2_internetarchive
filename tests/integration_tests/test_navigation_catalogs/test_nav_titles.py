...
NAV_EXPECTED = FIXTURES_DIR / "test_nav_titles.json"

GROUP_1 = {
    "path": FIXTURES_DIR / "test_items_metadata_nav_titles_g1.json",
    "title": "Title_A",
    "keyword": "firsttitle:a",
    "total": 652579
}

GROUP_2 = {
    "path": FIXTURES_DIR / "test_items_metadata_nav_titles_g2.json",
    "title": "Title_Z",
    "keyword": "firsttitle:z",
    "total": 36087
}
...
class TestNavigationLanguage(BaseNavigationTest):
    ...
    def catalog(self, catalog_factory):
        req = CatalogRequest(CatalogType.NAVIGATION.value, nav_key="page_titles")
        return catalog_factory.build_catalog(req).model_dump(mode="json")

    def test_metadata_title(self, catalog):
        assert catalog["metadata"]["title"] == "Browse by Titles"
    ...
    def test_links(self, catalog):
        ...
        assert next(l for l in catalog["links"] if l["rel"] == "self")["href"] == "/catalog?type=navigation&nav_key=page_titles"

    def test_navigation_items(self, catalog):
        assert len(catalog["navigation"]) == 36
